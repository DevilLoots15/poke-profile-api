 ...
 *
 * This function uses @napi-rs/canvas to load a background image (the template)
 * and then dynamically draws data (passed via query parameters) onto specific
 * coordinates, ensuring text is clean by first drawing 'eraser' rectangles.
 */
...
export default async function handler(req, res) {
  try {
    ...
    // IMPORTANT: Authenticate the request
    if (key !== API_KEY) {
      return res.status(401).json({ error: 'Unauthorized: Invalid API Key.' });
    }

    // 2. Get Data from Query Parameters
    const {
      id = 'N/A',
      dex = '0',
      caught = '0',
    ...
    // 3. Load the BLANK Template Image
    // NOTE: Please replace this URL with the final, direct link to your template image.
